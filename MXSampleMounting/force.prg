...
Global Double g_FSForces(NUM_FORCES)
...
Global Boolean g_FSCalibrateStatus
Global Integer g_FSTriggerForceName
Global Integer g_FSTriggerThreshold
Global Integer g_FSTriggerCompareType
...
Declare FSStartAcquisition, ASFSDLL, "FSStartAcquisition",(sampleFrequency As Integer, average As Integer) As Boolean
...
Declare FSCalibrate, ASFSDLL, "FSCalibrate", As Boolean
Declare FSReadForces, ASFSDLL, "FSReadForces",(ByRef forces() As Double) As Boolean
...
Function ForceInit
	...
	If FSStartAcquisition(28000, 1000) Then
...
Function ForceMeasureLoop
	...
	Do While 1
		...
		''Attempt to get exclusive access to g_FSForces
	    SyncLock FSSENSING_LOCK, SIG_LOCK_TIMEOUT
	    ''only access g_FSForces, g_FSForceTriggerStatus and g_FSTriggeredForces
	    ''if we get the lock
		If TW = False Then
			''measure the forces, and populate g_FSForces with new data
			FSReadForces(ByRef g_FSForces())
			...
			If (g_FSForceTriggerStatus <> 0 And FSForceTriggerStatus_old = 0) Then
				For i = 1 To NUM_FORCES Step 1
					g_FSTriggeredForces(i) = g_FSForces(i)
				Next
			EndIf
			''Release the lock
			SyncUnlock FSSENSING_LOCK
			''Give other threads a chance to get lock
			Wait .01
		Else
			Print "ForceMeasureLoop cannot get FSSENSING_LOCK"
		EndIf
	Loop
Fend
...
Function ReadForce(ByVal forceName As Integer) As Double
    ...
    ''Attempt to get exclusive access to g_FSForces
	SyncLock FSSENSING_LOCK, SIG_LOCK_TIMEOUT
    ''only access g_FSForces if we get the lock
	If TW = False Then
	    ReadForce = g_FSForces(ForceIndex)
	    SyncUnlock FSSENSING_LOCK
	Else
		''Failed to get the lock
		Print "ReadForce failed to get lock for force sensing"
	EndIf
Fend
...
Function ReadForces(ByRef returnForces() As Double)
	...
	If TW = False Then
	    For i = 1 To NUM_FORCES Step 1
    		''Copy force measurements to provided variable
	        returnForces(i) = g_FSForces(i)
	    Next
	    ''Release the lock
	    SyncUnlock FSSENSING_LOCK
	Else
	    ''Failed to get the lock
		Print "ReadForces failed to get lock for force sensing"
    EndIf
Fend
''Setup move until force trigger value
Function SetupForceTrigger(ByVal forceName As Integer, ByVal threshold As Real)
	''Print "SetupForceTrigger forceName: " + Str$(forceName) + " threshold: " + Str$(threshold)
    FSClearTrigger()
